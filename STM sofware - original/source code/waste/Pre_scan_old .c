...
//            Prescan2On = FALSE;
		...
		case IDC_OPENSERIAL:
			if(open_serial())
			{
				...
				{
					MessageBox(GetFocus(),"Unable to find DSP device at COM1","Comm Error!",MB_OK);
			...
			{
				MessageBox(GetFocus(),"Unable to establish serial connection on COM1", "Comm Error!",MB_OK);
				strcpy(firmware_vers_str,"Inaccessible");
			}
			break;
		case IDC_CLOSESERIAL:
			close_serial();
			break;
      }
      break;
