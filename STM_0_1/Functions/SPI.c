...
void InitSpi(void)
{
    ...
    // Set the baud rate
    SpiaRegs.SPIBRR.bit.SPI_BIT_RATE = SPI_BRRA;
    ...
    // Set the baud rate
    SpibRegs.SPIBRR.bit.SPI_BIT_RATE = SPI_BRRB;
...
//
// spi_dac - This function handle DAC SPI communication. 3 bytes only
//
byte spib_dac(byte sdata)
{
    Uint16 i;
    byte rdata;
    rdata.num = 3;
    ...
    for(i = 0; i < 3; i++)
    {
        rdata.arr[i] = spib_xmit(sdata.arr[i]);     // Transmit and receive 3 bytes of data
    }
...
//
// spi_bit20 - This function handle 20bit DAC SPI communication. 3 bytes only
//
byte spib_bit20(byte sdata)
{
    Uint16 i;
    byte rdata;
    rdata.num = 3;
