...
void scan()
{
    ...
    bool tip_protection, dir_x, dir_y;
    Uint16 move_delay, measure_delay, line_delay, stepsize, stepnum;
    ...
    line_delay = combine(serialIn(2));      // Acquire measure delay data
    ...
    feedback = ((flag & 0x04) >> 2);            // Feedback data for tip protection
    dir_x = ((flag & 0x10) == 0x10);            // X scan direction
    dir_y = ((flag & 0x20) == 0x20);            // Y scan direction
    ...
    for (i = 0; i < stepnum; i++)   // Line loop
    {
        ...
        if(dir_y){current_y = stored_y + (i * stepsize);}
        else{current_y = stored_y - (i * stepsize);}
        rampTo_S(channel_y, current_y, 1, move_delay);         // Ramp to next line
        protectTip(tip_protection, tip_protect_data, true);    // Undo tip protection
        ...
        protectTip(tip_protection, tip_protect_data, false);    // Do tip protection
        if (stopped || (! condition)){break;}                   // If stopped or scan protection tells not continue, break loop
