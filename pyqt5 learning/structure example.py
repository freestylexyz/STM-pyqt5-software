...
class myMainMenu(QMainWindow, Ui_HoGroupSTM):
    ...
    def open_setting(self):
        self.seting.init_setting(many varaibles)
        self.setting.show()
    ...
    def setting_initdsp_slot(self):
        self.dsp.init_dsp()     # reinitial dsp
        self.setting            # change setting view
...
class mySetting(QWidget, Ui_Setting):
    ...
    #     # not called for now
    def init_UI(self):
        self.initButton.clicked.connect(self.dsp.init_dsp)
        self.loadButton.clicked.connect(self.dsp.init_dsp)
