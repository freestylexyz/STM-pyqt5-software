...
class myDSP(QObject):
    ...
    #
    def osc_C(self, addr, n):
        if self.ok():
            self.idling = False
            self.ser.write(int(0xc0).to_bytes(1, byteorder="big"))        # 0xc0 for continuous oscilloscope
            self.ser.write(int(addr & 0xff).to_bytes(1, byteorder="big"))   # Send adress
            self.ser.write(int(n & 0xffff).to_bytes(2, byteorder="big"))    # Send average number
            # If receive start command
            if int.from_bytes(self.ser.read(1) ,"big") == 0xf0:
                self.stop = False        # Set stop flag to false
                while True:        
    ...
    def osc_N(self, addr, n):
        ...
        if self.ok():
            ...
            self.ser.write(int(addr).to_bytes(1, byteorder="big"))   # send adress
            self.ser.write(int(n).to_bytes(2, byteorder="big"))    # send average number
            for i in range(n):
    ...
    def rampMeasure(self, channel, target, step, move_delay, measure_delay, command_list, data_list):
        if self.ok():
            ...
            seqnum = len(command_list) & 0x3f
            
            command = np.array(command_list)
            readnum = len(command[(command >= 0xc000) and (command <= 0xdc00)])
            
            
            if channel == 0x20:
                current = self.last20bit
            else:
                current = self.lastdac[channel -16]
            ...
            self.ser.write(int(seqnum).to_bytes(1, byteorder="big"))         # Send sequence number
            # Send sequence command and data
            for i in range(seqnum):
                self.ser.write(int(command_list[i] & 0xffff).to_bytes(2, byteorder="big"))   # Send command list
                self.ser.write(int(data_list[i] & 0x0fffff).to_bytes(3, byteorder="big"))    # Send data list
            ...
            if int.from_bytes(self.ser.read(1) ,"big") == 0xf0:
                ...
                # Return data to update last output
                rdata = int.from_bytes(self.ser.read(3) ,"big")
                self.update_last(channel, rdata)
                self.stop = True    # Set stop flag to true 
            self.idling = True
    ...
    def giantStep(self, channel, x, z, delay1, delay, gx, gz, stepnum):
        if self.ok():
            ...
            self.ser.write(int(gx).to_bytes(2, byteorder="big"))        # Send acceleration or x ramp
            self.ser.write(int(gz).to_bytes(2, byteorder="big"))        # Send acceleration or z ramp
