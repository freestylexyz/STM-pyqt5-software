...
class myDSP(QObject):
    ...
    def __init__(self, parent = None):
        super(myDSP, self).__init__(parent)    # DanB: (parent, port, baudrate) --> (parent)
        ...
        self.intial_dsp()               # DanB: added line
    ...
    def intial_dsp(self, port = 'com1', baudrate = 38400):
        ...
        try:
            self.open = True                # DanB: move this line upward (if_open: Flase --> True)
            self.version()                  # DanB: move this line upward (if_version_obtained: Flase --> True)
            self.status()                   # DanB: move this line upward (if_status_obtained: Flase --> True)
    ...
    def status(self):
        if self.open and self.idling:
            ...
            if True:
                ...
                # print("in status: ",self.status_obtained) # DanB: debug line
            else:
...
if __name__ == "__main__":
    dsp = myDSP()
    print("if_idling: ",dsp.idling)
    print("if_open: ",dsp.open)
    print("if_version_obtained: ",dsp.version_obtained)
    print("if_status_obtained: ",dsp.status_obtained)
    print("if_succeed: ",dsp.succeed)
    print("offset: ",dsp.offset)
    dsp.offset[-1] = 6.66
    print("Isetoff: ",dsp.offset[-1])
