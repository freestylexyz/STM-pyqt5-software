...
class myDSP(QObject):
    ...
    def init_dsp(self, initOut):
        try:
            ...
            self.last20bit = 0x80000        # Last output of 20bit DAC
            self.lastdigital = [False] * 6  # 0 - 5 : bias dither, z dither, feedback, retract, coarse, translation
            self.lastgain = [1] * 4         # 0 -> gain 10.0, 1 -> gain 1.0, 3 -> gain 0.1, Z1 gain is different from
            # others, 3 -> gain 10.0, 1 -> gain 1.0, 0 -> gain 0.1
            self.offset = [0] * 16          # 0 - 14 are bias offset for different range, 15 is Iset offset
            self.port = 'com1'
            self.baudrate = 38400
        if not self.succeed:
    ...
    # Read serial data after check available byte, max t s before abort, protected method, internal use only
    # Return if reading successful and read data
    def __readt(self, n, t):
        if self.open:
            ...
            # If get enough charters, reading susses and read the # of byte
    ...
    def init_output(self):
        if self.ok():
            ...
            # Initial all ADC range to +/- 10.24V
            for i in range(8):
                self.adc_W(((i + 5) << 1), 0)
    ...
    def lastdigital_O(self):
        if self.ok():
            self.idling = False
            self.ser.write((0x72).to_bytes(1, byteorder='big'))  # 0x72 lastdigital_O command
    ...
    # adc - This function command dsp to execute 1 conversion of specific channel and return conversion result
    def adc(self, addr):
        if self.ok():
            self.idling = False
    ...
    # adc_N - This function command dsp to execute N conversion of specific channel and return averaged conversion
    # result
    ...
    def adc_W(self, addr, data):
        if self.ok():
            ...
            self.ser.write(int(addr).to_bytes(1, byteorder="big"))  # send address
            self.ser.write(int(data).to_bytes(1, byteorder="big"))  # send address
            self.idling = True
    ...
    def adc_R(self, addr):
        if self.ok():
            ...
            self.ser.write(int(addr & 0xfe).to_bytes(1, byteorder="big"))  # send address
    ...
    def digital_o(self, channel, data):
        if self.ok():
            ...
            self.idling = True
            self.lastdigital[channel] = bool(data)  # Update last digital variable
    ...
    def bit20_R(self, addr):
        if self.ok():
            ...
            self.ser.write(int(addr & 0x70).to_bytes(1, byteorder="big"))  # send address
    ...
    def dac_W(self, addr, data):
        if self.ok():
            ...
            self.ser.write(int(sdata).to_bytes(3, byteorder="big"))  # send data
            self.idling = True
            # Update last dac variable if writing to DAC DAC register
            if (0x30 & addr) == 0x10:
    ...
    def dac_R(self, addr):
        if self.ok():
            ...
            self.ser.write(int(addr & 0x3f).to_bytes(1, byteorder="big"))  # send address
            rdata = int.from_bytes(self.ser.read(3),
                                   "big")  # read data back, first data should be address with R/W flag
    ...
    # zAuto0 - This function command DSP do zAuto0_DSP function
    def zAuto0(self):
        if self.ok():
            self.idling = False
    ...
    # iAuto - This function command DSP do iAuto_DSP function
    def iAuto(self):
        if self.ok():
    ...
    # limit_out - limit output based on channel
    ...
    # osc_N - This function execute N samples oscilloscope function
    ...
    def rampTo(self, channel, target, step, delay, limit, checkstop):
        if self.ok():
            ...
            self.ser.write(
                int(limit).to_bytes(2, byteorder="big"))  # Send out limit for crash protection and check stop flag
            ...
            if int.from_bytes(self.ser.read(1), "big") == 0xf0:
                ...
                # Use returned data to update last output global variable
    ...
    def giantStep(self, channel, x, z, delay, g, stepnum):
        if self.ok():
            ...
            if int.from_bytes(self.ser.read(1), "big") == 0xf0:
                self.stop = False  # Set stop flag to false
                i = 0  # Initial giant step counter
    ...
    def deposition(self, read_ch, read_flag, read_delay, read_delay2, read_num,
        ...
        if self.ok():
            ...
            read_delay2 = read_delay2 & 0xffff
            ...
            self.ser.write(int(read_delay2).to_bytes(2, byteorder="big"))  # Send read delay 2
